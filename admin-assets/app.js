...
const ui={list:$('#list'),status:$('#status'),missing:$('#missingCount'),staged:$('#stagedCount'),assigned:$('#assignedCount'),missingTab:$('#missingTab'),assignedTab:$('#assignedTab'),search:$('#searchInput'),file:$('#fileInput'),commit:$('#commitButton')};
let words=[], queue=[], view='missing', selected=null, busy=false, query='';
...
function assignedWords(){const q=queued();const staged=queue.map(stage=>({...stage,stage}));const existing=words.filter(hasAsset).filter(w=>!q.has(w.word)).map(w=>({...w,scene:sceneFor(w.masked,w.word)}));return [...staged,...existing].sort((a,b)=>a.word.localeCompare(b.word,'pl'))}
function matchesSearch(item){
  if(!query)return true;
  const asset=item.filename||item.scene?.asset||'';
  return searchKey([item.word,item.category,asset].join(' ')).includes(query);
}
...
function render(){
  ...
  const data=(view==='missing'?missing:assigned).filter(matchesSearch);
  const nodes=data.length?data.map(rowFor):[Object.assign(document.createElement('div'),{className:'empty',textContent:query?'Brak wyników.':(view==='missing'?'Brak słów bez grafiki.':'Brak przypisanych grafik.')})];
...
ui.missingTab.onclick=()=>setView('missing');ui.assignedTab.onclick=()=>setView('assigned');ui.search.oninput=()=>{query=searchKey(ui.search.value.trim());render()};ui.file.onchange=onFile;ui.commit.onclick=commit;
